...
LATITUDE = 45.07
LONGITUDE = 7.54
FILE_LAST_HOUR = "ultima_ora_icon_ch2_stac_24h.txt"
RUN_DURATION = 120
START_DELAY = 1
...
def genera_album_24h(dt_run_utc: datetime, nome_run: str):
    ...
    for h_start, h_end in intervals:
        ...
        if h_start == 0:
            ...
            prec_diff = prec_mean.sel(lead_time=np.timedelta64(h_end, 'h')) - prec_mean.sel(lead_time=np.timedelta64(h_start, 'h'))
        ...
        filename = f"accumulo_{h_start}_{h_end}.png"
        chart.save(filename)
        percorsi_foto.append(filename)
        
        plt.close(chart.fig)
    
    caption_album = f"ICON-CH2 EPS: Accumuli 24h e spezzoni\nRun {nome_run}"
...
def main():
    ...
    if is_new:
        print(f"🚀 Lancio generazione Album giornalieri/spezzoni per il RUN {nome_run} ({dt_run_utc})")
