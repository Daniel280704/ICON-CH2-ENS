...
LATITUDE = 45.07
LONGITUDE = 7.54
FILE_LAST_HOUR = "ultima_ora_icon_ch2_lfc.txt"
RUN_DURATION = 120
START_DELAY = 1
...
def invia_album_telegram(file_paths: list, caption: str):
    ...
    thread_id = os.getenv("TELEGRAM_THREAD_ID_28")
...
def genera_album_orari(dt_run_utc: datetime, nome_run: str):
    ...
    # LFC scale 0 - 6000 (salti di 500)
    my_levels = [0, 500, 1000, 1500, 2000, 2500, 3000, 3500, 4000, 4500, 5000, 5500, 6000]
    my_colors = ["#a50026", "#d73027", "#f46d43", "#fdae61", "#fee090", "#ffffbf", "#e0f3f8", "#abd9e9", "#74add1", "#4575b4", "#313695", "#08306b"]
    ...
    for block_name, ore_list in blocchi.items():
        ...
        try:
            print(f"  ⬇️  Scarico dati LFC_ML per {len(ore_list)} ore...")
        ...
        for h in ore_list:
            ...
            title = f"ICON-CH2 EPS - LFC Mean (m)\nRun: {dt_run_utc.strftime('%d/%m/%Y %H:%M UTC')} | {str_valida}"
            chart.title(title)
            chart.legend(label="LFC_ML (m)")
        ...
        caption_album = f"ICON-CH2 EPS: Dettaglio LFC\n{block_name}\nRun {nome_run}"
