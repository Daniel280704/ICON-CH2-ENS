...
FILE_LAST_HOUR = "ultima_ora_icon_ch2_snow_prob.txt" 
RUN_DURATION = 120
START_DELAY = 1
...
def estrai_limiti_run(hourly_data: dict, ref_param: str, utc_offset_sec: int) -> tuple[bool, str, datetime]:
    ...
    if os.path.exists(FILE_LAST_HOUR):
        ...
        if ultima_ora_valida_str <= ultima_ora_salvata:
            print(f"✅ Run ICON-CH2 {nome_run} (Probabilità Neve) già elaborato (Ultimo blocco: {ultima_ora_valida_str}).")
            return False, "", None
...
def genera_album_orari(dt_run_utc: datetime, nome_run: str):
    ...
    for block_name, ore_list in blocchi.items():
        print(f"\nGenerazione album probabilità neve: {block_name}")
        ...
        for h in ore_list:
            if h == 1:
                snow_diff = tot_snow.sel(lead_time=np.timedelta64(h, 'h'))
            else:
            ...
            filename = f"neve_oraria_prob_{h}.png"
            chart.save(filename)
            percorsi_foto.append(filename)
            
            plt.close(chart.fig)
        
        caption_album = f"ICON-CH2 EPS: Probabilità Neve oraria ≥ 0.5 mm eq. acqua\n{block_name}\nRun {nome_run}"
...
def main():
    print("Cerco l'ultimo run completo ICON-CH2 via Open-Meteo per le probabilità di neve...")
    ...
    if is_new:
        print(f"🚀 Lancio generazione Probabilità Neve Orarie ICON-CH2 per il RUN {nome_run} ({dt_run_utc})")
