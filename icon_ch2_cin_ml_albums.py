...
def estrai_limiti_run(hourly_data: dict, ref_param: str) -> tuple[bool, str, datetime]:
    ...
    if os.path.exists(FILE_LAST_HOUR):
        ...
        if ultima_ora_valida_str <= ultima_ora_salvata:
            print(f"✅ Run ICON-CH2 CIN_ML {nome_run} già elaborato.")
            return False, "", None
...
def genera_album_cin_ml(dt_run_utc: datetime, nome_run: str):
    rome_tz = pytz.timezone("Europe/Rome")
    ...
    try:
        print(f"  ⬇️  Scarico dati CIN_ML per le 120 ore...")
    ...
    for day_str, intervals in intervals_by_day.items():
        ...
        for h_start, h_end in intervals:
            ...
            title = f"ICON-CH2 EPS - CIN_ML MAX (Media dei Massimi)\n{day_str} | Fascia {orario_str}\nRun: {dt_run_utc.strftime('%d/%m %H:%M UTC')}"
            
            chart.title(title)
            chart.legend(label="CIN_ML (J/kg)")

            filename = f"cin_ml_max_{h_start}_{h_end}.png"
            chart.save(filename)
            percorsi_foto.append(filename)
            
            plt.close(chart.fig)
        
        caption_album = f"ICON-CH2 EPS: CIN_ML (Inibizione Connettiva Mixed Layer) MAX\nFasce triorarie del {day_str}\nRun {nome_run}"
...
def main():
    ...
    if is_new:
        print(f"🚀 Lancio generazione Album CIN_ML per il RUN {nome_run} ({dt_run_utc})")
        genera_album_cin_ml(dt_run_utc, nome_run)
