...
LATITUDE = 45.07
LONGITUDE = 7.54
FILE_LAST_HOUR = "ultima_ora_icon_ch2_cdct.txt"
RUN_DURATION = 120
START_DELAY = 1
...
def genera_album_orari(dt_run_utc: datetime, nome_run: str):
    ...
    my_levels = [0, 1000, 2000, 3000, 4000, 5000, 6000, 7000, 8000, 9000, 10000, 11000, 12000, 13000, 14000, 15000]
    my_colors = ["#f7fcf0", "#e0f3db", "#ccebc5", "#a8ddb5", "#7bccc4", "#4eb3d3", "#2b8cbe", "#0868ac", "#084081", "#fdbb84", "#fc8d59", "#ef6548", "#d7301f", "#b30000", "#7f0000"]
    ...
    for block_name, ore_list in blocchi.items():
        ...
        try:
            print(f"  ⬇️  Scarico dati CDCT per {len(ore_list)} ore...")
        ...
        for h in ore_list:
            ...
            title = f"ICON-CH2 EPS - Cloud Top (m)\nRun: {dt_run_utc.strftime('%d/%m/%Y %H:%M UTC')} | {str_valida}"
            chart.title(title)
            chart.legend(label="CDCT (m)")
        ...
        caption_album = f"ICON-CH2 EPS: Dettaglio EQ Level (Cloud Top)\n{block_name}\nRun {nome_run}"
