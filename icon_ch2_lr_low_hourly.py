...
LATITUDE = 45.07
LONGITUDE = 7.54
FILE_LAST_HOUR = "ultima_ora_icon_ch2_lr_low.txt"
RUN_DURATION = 120
START_DELAY = 1
...
def estrai_limiti_run(hourly_data: dict, ref_param: str, utc_offset_sec: int) -> tuple[bool, str, datetime]:
    ...
    if os.path.exists(FILE_LAST_HOUR):
        ...
        if ultima_ora_valida_str <= ultima_ora_salvata:
            print(f"✅ Run ICON-CH2 {nome_run} Low-Level LR già elaborato (Ultimo blocco: {ultima_ora_valida_str}).")
            return False, "", None
...
def invia_album_telegram(file_paths: list, caption: str):
    ...
    thread_id = os.getenv("TELEGRAM_THREAD_ID_5861")
    ...
    try:
        ...
        print(f"📸 Album Low-Level LR inviato ({len(file_paths)} mappe).")
    except Exception as e: print(f"Errore invio: {e}")
    finally:
...
def genera_album_orari(dt_run_utc: datetime, nome_run: str):
    ...
    # Stessa palette del Mid-Level, espansa per i bassi strati (super-adiabatici > 9.8)
    my_levels = [4.0, 5.0, 6.0, 6.5, 7.0, 7.5, 8.0, 9.0, 9.8, 11.0]
    my_colors = [
        "#228b22", # Verde scuro 
        "#3cb371", # Verde medio
        "#a0e6a0", # Verde chiaro 
        ...
        "#44aaff", # Azzurro 
        "#0000ff", # Blu 
        "#000080", # Blu scuro/Navy 
        "#800080", # Viola
        "#4b0082"  # Indaco (Instabilità secca estrema)
    ]
    ...
    for block_name, ore_list in blocchi.items():
        print(f"\nGenerazione album Low-Level LR per {block_name}")
        percorsi_foto = []
        ...
        for h in ore_list:
            ...
            try:
                ...
                t_850 = interpolate_k2p(field=data_t, mode="linear_in_lnp", p_field=data_p, p_tc_values=[850], p_tc_units="hPa")
                t_1000 = interpolate_k2p(field=data_t, mode="linear_in_lnp", p_field=data_p, p_tc_values=[1000], p_tc_units="hPa")

            except Exception as e:
                print(f"Salto l'ora {h} causa errore: {e}")
                continue

            t_850_geo = regrid.iconremap(t_850.squeeze(drop=True), destination)
            t_1000_geo = regrid.iconremap(t_1000.squeeze(drop=True), destination)

            # --- CALCOLO SPESSORE GEOMETRICO (Hydrostatic Equation per 1000/850 hPa) ---
            t_mean = (t_850_geo.values + t_1000_geo.values) / 2.0
            dz_meters = 4.757 * t_mean

            # --- CALCOLO LAPSE RATE (°C/km) ---
            lr_np = (t_1000_geo.values - t_850_geo.values) / dz_meters * 1000.0

            lr_xr = t_850_geo.copy(data=lr_np)

            chart = earthkit.plots.Map(domain=domain)
            chart.grid_cells(lr_xr, x="lon", y="lat", style=Style(colors=my_colors, levels=my_levels))
            ...
            title = f"ICON-CH2 EPS - Low-Level Lapse Rate 1000-850 hPa (°C/km)\nMEDIA SCENARI | Run: {dt_run_utc.strftime('%d/%m/%Y %H:%M UTC')}\n{str_valida}"
            
            chart.title(title)
            chart.legend(label="Lapse Rate (°C / km)")

            filename = f"lr_low_{h}.png"
            chart.save(filename)
            percorsi_foto.append(filename)
            plt.close(chart.fig)
            
            del data_p, data_t, t_850, t_1000, t_850_geo, t_1000_geo, t_mean, lr_np, lr_xr
            gc.collect()
        
        if percorsi_foto:
            caption_album = f"Low-Level LR 1000-850 hPa Media EPS\n{block_name}\nRun {nome_run}"
...
def main():
    print("Cerco l'ultimo run completo ICON-CH2 per Low-Level LR (Step 3h, Media Scenari)...")
    ...
    if is_new:
        print(f"🚀 Lancio generazione Album Low-Level LR per il RUN {nome_run}")
