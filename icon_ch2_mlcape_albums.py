...
def genera_album_mlcape(dt_run_utc: datetime, nome_run: str):
    ...
    try:
        print(f"  ⬇️  Scarico dati CAPE_ML per le 120 ore...")
        mlcape_data = scarica_variabile_con_retry(req)
    ...
    for day_str, intervals in intervals_by_day.items():
        ...
        for h_start, h_end in intervals:
            ...
            mlcape_interval = mlcape_data.sel(lead_time=hours_slice).max(dim="lead_time").mean(dim="eps")

            mlcape_geo = regrid.iconremap(mlcape_interval, destination)
            ...
            title = f"ICON-CH2 EPS - CAPE ML (J/kg) MAX (Media dei Massimi)\n{day_str} | Fascia {orario_str}\nRun: {dt_run_utc.strftime('%d/%m %H:%M UTC')}"
            
            chart.title(title)
            chart.legend(label="CAPE ML (J/kg)")

            filename = f"mlcape_max_{h_start}_{h_end}.png"
            chart.save(filename)
            percorsi_foto.append(filename)
            
            plt.close(chart.fig)
        
        caption_album = f"ICON-CH2 EPS: CAPE ML (Instabilità Mixed-Layer) MAX\nFasce triorarie del {day_str}\nRun {nome_run}"
