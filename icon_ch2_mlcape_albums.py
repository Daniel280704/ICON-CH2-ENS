...
LATITUDE = 45.07
LONGITUDE = 7.54
FILE_LAST_HOUR = "ultima_ora_icon_ch2_mlcape.txt"
...
def estrai_limiti_run(hourly_data: dict, ref_param: str) -> tuple[bool, str, datetime]:
    ...
    if os.path.exists(FILE_LAST_HOUR):
        ...
        if ultima_ora_valida_str <= ultima_ora_salvata:
            print(f"✅ Run ICON-CH2 MLCAPE {nome_run} già elaborato.")
            return False, "", None
...
def invia_album_telegram(file_paths: list, caption: str):
    ...
    thread_id = os.getenv("TELEGRAM_THREAD_ID_8")
...
def genera_album_mlcape(dt_run_utc: datetime, nome_run: str):
    rome_tz = pytz.timezone("Europe/Rome")
    ...
    # Scala colori MLCAPE dettagliata: 200-5000 J/kg
    ...
    for block_name, ore_list in blocchi.items():
        print(f"\n📊 Generazione album MLCAPE: {block_name}")
        ...
        try:
            print(f"  ⬇️  Scarico dati CAPE_ML per {len(ore_list)} ore...")
            mlcape_raw = ogd_api.get_from_ogd(req)
            mlcape_mean = mlcape_raw.mean(dim="eps")
        ...
        for h in ore_list:
            mlcape_step = mlcape_mean.sel(lead_time=np.timedelta64(h, 'h'))
            mlcape_geo = regrid.iconremap(mlcape_step, destination)

            chart = earthkit.plots.Map(domain=domain)
            chart.grid_cells(mlcape_geo, x="lon", y="lat", style=Style(colors=my_colors, levels=my_levels))
            ...
            chart.title(f"ICON-CH2 EPS - CAPE ML (J/kg)\nRun: {dt_run_utc.strftime('%d/%m/%Y %H:%M UTC')} | {str_valida}")
            chart.legend(label="CAPE ML (J/kg)")
            
            f_name = f"mlcape_{h}.png"
            chart.save(f_name)
...
def main():
    ...
    if is_new:
        print(f"🚀 Lancio generazione Album MLCAPE per il RUN {nome_run} ({dt_run_utc})")
        genera_album_mlcape(dt_run_utc, nome_run)
