...
LATITUDE = 45.07
LONGITUDE = 7.54
FILE_LAST_HOUR = "ultima_ora_icon_ch2_snow_stac.txt"
RUN_DURATION = 120
START_DELAY = 1
...
def invia_album_telegram(file_paths: list, caption: str):
    ...
    thread_id = os.getenv("TELEGRAM_THREAD_ID_54")
...
def genera_album_orari(dt_run_utc: datetime, nome_run: str):
    ...
    for block_name, ore_list in blocchi.items():
        print(f"\nGenerazione album neve: {block_name}")
        ...
        for h in ore_list:
            if h == 1:
                snow_diff = snow_mean.sel(lead_time=np.timedelta64(h, 'h'))
            else:
                snow_diff = snow_mean.sel(lead_time=np.timedelta64(h, 'h')) - snow_mean.sel(lead_time=np.timedelta64(h-1, 'h'))
            ...
            title = f"ICON-CH2 EPS - Nevicata Oraria (mm eq. acqua/h)\nRun: {dt_run_utc.strftime('%d/%m/%Y %H:%M UTC')} | {str_valida}"
            chart.title(title)
            chart.legend(label="Neve (mm eq. acqua/h)")

            filename = f"neve_oraria_{h}.png"
            chart.save(filename)
            percorsi_foto.append(filename)
            
            plt.close(chart.fig)
        
        caption_album = f"❄️ ICON-CH2 EPS: Dettaglio Neve Oraria\n{block_name}\nRun {nome_run}"
...
def main():
    print("Cerco l'ultimo run completo ICON-CH2 via Open-Meteo per la Neve...")
    ...
    if is_new:
        print(f"🚀 Lancio generazione Album Orari Neve ICON-CH2 per il RUN {nome_run} ({dt_run_utc})")
