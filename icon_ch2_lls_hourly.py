...
LATITUDE = 45.07
LONGITUDE = 7.54
FILE_LAST_HOUR = "ultima_ora_icon_ch2_lls.txt"
RUN_DURATION = 120
START_DELAY = 1
...
def estrai_limiti_run(hourly_data: dict, ref_param: str, utc_offset_sec: int) -> tuple[bool, str, datetime]:
    ...
    if actual_points < expected_points: return False, "", None
        
    if os.path.exists(FILE_LAST_HOUR):
        with open(FILE_LAST_HOUR, "r") as f:
            ultima_ora_salvata = f.read().strip()
        if ultima_ora_valida_str <= ultima_ora_salvata: return False, "", None
...
def invia_album_telegram(file_paths: list, caption: str):
    ...
    try:
        ...
        print(f"📸 Album LLS inviato ({len(file_paths)} mappe).")
    except Exception as e: print(f"Errore invio: {e}")
    finally:
...
def genera_album_orari(dt_run_utc: datetime, nome_run: str):
    ...
    my_levels = [2, 5, 7.5, 10, 12.5, 15, 20, 25, 30]
    ...
    target_coords_1_5km = TargetCoordinates(
    ...
    for block_name, ore_list in blocchi.items():
        print(f"\nGenerazione album LLS per {block_name}")
        percorsi_foto = []
        ...
        for h in ore_list:
            ...
            try:
                print(f"  ⬇️  Scarico dati LLS (U, V, U_10M, V_10M) per l'ora {h}...")
                ...
                u_1_5km = interpolate_k2any(field=data_u_ml, mode="high_fold", tc_field=HFL, tc=target_coords_1_5km, h_field=HFL)
                v_1_5km = interpolate_k2any(field=data_v_ml, mode="high_fold", tc_field=HFL, tc=target_coords_1_5km, h_field=HFL)

            except Exception as e:
                print(f"  ❌ Salto l'ora {h} dopo 3 tentativi. Errore: {e}")
                continue

            u_up = u_1_5km.squeeze(drop=True)
            v_up = v_1_5km.squeeze(drop=True)
            ...
            title = f"ICON-CH2 EPS - Low Level Shear SFC-1.5 km (m/s)\nMEDIA SCENARI | Run: {dt_run_utc.strftime('%d/%m/%Y %H:%M UTC')}\n{str_valida}"
            
            chart.title(title)
            chart.legend(label="LLS (m/s)")

            filename = f"lls_{h}.png"
            chart.save(filename)
            percorsi_foto.append(filename)
            plt.close(chart.fig)
            
            del data_u_ml, data_v_ml, data_u_sfc, data_v_sfc, u_1_5km, v_1_5km, u_up, v_up, u_sfc, v_sfc, shear_geo
            gc.collect()
        
        if percorsi_foto:
            caption_album = f"LLS Media EPS (SFC-1500m Geometrici)\n{block_name}\nRun {nome_run}"
...
def main():
    print("Cerco l'ultimo run completo ICON-CH2 per LLS (Step 3h, Media Scenari)...")
    ...
    if is_new:
        print(f"🚀 Lancio generazione Album LLS per il RUN {nome_run}")
