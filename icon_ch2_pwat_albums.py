...
def estrai_limiti_run(hourly_data: dict, ref_param: str) -> tuple[bool, str, datetime]:
    ...
    if os.path.exists(FILE_LAST_HOUR):
        ...
        if ultima_ora_valida_str <= ultima_ora_salvata:
            print(f"✅ Run ICON-CH2 PWAT {nome_run} già elaborato.")
            return False, "", None
...
def main():
    ...
    if is_new:
        print(f"🚀 Lancio generazione Album PWAT per il RUN {nome_run} ({dt_run_utc})")
        genera_album_pwat(dt_run_utc, nome_run)
