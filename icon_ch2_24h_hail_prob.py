...
LATITUDE = 45.07
LONGITUDE = 7.54
FILE_LAST_HOUR = "ultima_ora_icon_ch2_hail_prob_24h.txt"
RUN_DURATION = 120
START_DELAY = 1
...
def estrai_limiti_run(hourly_data: dict, ref_param: str, utc_offset_sec: int) -> tuple[bool, str, datetime]:
    ...
    if os.path.exists(FILE_LAST_HOUR):
        ...
        if ultima_ora_valida_str <= ultima_ora_salvata:
            print(f"✅ Run ICON-CH2 {nome_run} già elaborato per la probabilità grandine (Ultimo blocco: {ultima_ora_valida_str}).")
            return False, "", None
...
def main():
    print("Cerco l'ultimo run completo ICON-CH2 via Open-Meteo per la probabilità grandine...")
    ...
    if is_new:
        print(f"🚀 Lancio generazione Album Probabilità Grandine per il RUN {nome_run} ({dt_run_utc})")
