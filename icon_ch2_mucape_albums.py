...
LATITUDE = 45.07
LONGITUDE = 7.54
FILE_LAST_HOUR = "ultima_ora_icon_ch2_mucape.txt"
...
def estrai_limiti_run(hourly_data: dict, ref_param: str) -> tuple[bool, str, datetime]:
    ...
    if os.path.exists(FILE_LAST_HOUR):
        ...
        if ultima_ora_valida_str <= ultima_ora_salvata:
            print(f"✅ Run ICON-CH2 MUCAPE {nome_run} già elaborato.")
            return False, "", None
...
def invia_album_telegram(file_paths: list, caption: str):
    ...
    thread_id = os.getenv("TELEGRAM_THREAD_ID_7")
...
def genera_album_mucape(dt_run_utc: datetime, nome_run: str):
    rome_tz = pytz.timezone("Europe/Rome")
    ...
    # Scala colori MUCAPE dettagliata: 200-5000 J/kg
    ...
    for block_name, ore_list in blocchi.items():
        print(f"\n📊 Generazione album MUCAPE: {block_name}")
        ...
        try:
            print(f"  ⬇️  Scarico dati MUCAPE per {len(ore_list)} ore...")
            mucape_raw = ogd_api.get_from_ogd(req)
            mucape_mean = mucape_raw.mean(dim="eps")
        ...
        for h in ore_list:
            mucape_step = mucape_mean.sel(lead_time=np.timedelta64(h, 'h'))
            mucape_geo = regrid.iconremap(mucape_step, destination)

            chart = earthkit.plots.Map(domain=domain)
            chart.grid_cells(mucape_geo, x="lon", y="lat", style=Style(colors=my_colors, levels=my_levels))
            ...
            chart.title(f"ICON-CH2 EPS - MUCAPE (J/kg)\nRun: {dt_run_utc.strftime('%d/%m/%Y %H:%M UTC')} | {str_valida}")
            chart.legend(label="MUCAPE (J/kg)")
            
            f_name = f"mucape_{h}.png"
            chart.save(f_name)
        ...
        if percorsi_foto:
            caption_album = f"⚡ ICON-CH2 EPS: MUCAPE (Instabilità)\n🗓 {block_name}\n⚙️ Run {nome_run}"
...
def main():
    ...
    if is_new:
        print(f"🚀 Lancio generazione Album MUCAPE per il RUN {nome_run} ({dt_run_utc})")
        genera_album_mucape(dt_run_utc, nome_run)
