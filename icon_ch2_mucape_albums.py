...
def genera_album_mucape(dt_run_utc: datetime, nome_run: str):
    ...
    try:
        print(f"  ⬇️  Scarico dati MUCAPE per le 120 ore...")
        mucape_data = scarica_variabile_con_retry(req)
    ...
    for day_str, intervals in intervals_by_day.items():
        ...
        for h_start, h_end in intervals:
            ...
            mucape_interval = mucape_data.sel(lead_time=hours_slice).max(dim="lead_time").mean(dim="eps")

            mucape_geo = regrid.iconremap(mucape_interval, destination)
            ...
            title = f"ICON-CH2 EPS - MUCAPE (J/kg) MAX (Media dei Massimi)\n{day_str} | Fascia {orario_str}\nRun: {dt_run_utc.strftime('%d/%m %H:%M UTC')}"
            
            chart.title(title)
            chart.legend(label="MUCAPE (J/kg)")

            filename = f"mucape_max_{h_start}_{h_end}.png"
            chart.save(filename)
            percorsi_foto.append(filename)
            
            plt.close(chart.fig)
        
        caption_album = f"ICON-CH2 EPS: MUCAPE (Instabilità) MAX\nFasce triorarie del {day_str}\nRun {nome_run}"
