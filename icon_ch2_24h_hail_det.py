...
def genera_album_24h(dt_run_utc: datetime, nome_run: str):
    ...
    for h_start, h_end in intervals:
        ...
        title = f"ICON-CH2 - Grandine Massima Attesa (cm)\nCORSA DETERMINISTICA | Run: {dt_run_utc.strftime('%d/%m/%Y %H:%M UTC')}\n{str_valida}"
        
        chart.title(title)
        chart.legend(label="Grandine (cm)")

        filename = f"hail_max_det_{h_start}_{h_end}.png"
        chart.save(filename)
...
def main():
    ...
    if is_new:
        print(f"🚀 Lancio generazione Album Grandine giornalieri (Deterministico) per il RUN {nome_run} ({dt_run_utc})")
