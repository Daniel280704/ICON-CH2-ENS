...
LATITUDE = 45.07
LONGITUDE = 7.54
FILE_LAST_HOUR = "ultima_ora_icon_ch2_fog_prob_24h.txt"
RUN_DURATION = 120
START_DELAY = 1
...
def estrai_limiti_run(hourly_data: dict, ref_param: str, utc_offset_sec: int) -> tuple[bool, str, datetime]:
    ...
    if os.path.exists(FILE_LAST_HOUR):
        ...
        if ultima_ora_valida_str <= ultima_ora_salvata:
            print(f"✅ Run ICON-CH2 {nome_run} già elaborato per la probabilità nebbia (Ultimo blocco: {ultima_ora_valida_str}).")
            return False, "", None
...
def invia_album_telegram(file_paths: list, caption: str):
    ...
    thread_id = os.getenv("TELEGRAM_THREAD_ID_52")
...
def genera_album_giornalieri(dt_run_utc: datetime, nome_run: str):
    ...
    for day_str, intervals in intervals_by_day.items():
        ...
        for h_start, h_end in intervals:
            ...
            chart.grid_cells(fog_geo, x="lon", y="lat", style=Style(colors=my_colors, levels=my_levels))
...
def main():
    print("Cerco l'ultimo run completo ICON-CH2 via Open-Meteo per la probabilità nebbia...")
    ...
    if is_new:
        print(f"🚀 Lancio generazione Album Probabilità Nebbia per il RUN {nome_run} ({dt_run_utc})")
