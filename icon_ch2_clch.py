...
LATITUDE, LONGITUDE, FILE_LAST_HOUR = 45.07, 7.54, "ultima_ora_icon_ch2_clch.txt"
...
def invia_album(file_paths, caption):
    t, c, th = os.getenv("TELEGRAM_TOKEN"), os.getenv("TELEGRAM_CHAT_ID"), os.getenv("TELEGRAM_THREAD_ID_18")
...
def genera(dt_utc, n_run):
    ...
    for bn, ore in raggruppa(dt_loc).items():
        req = ogd_api.Request("ogd-forecasting-icon-ch2", "CLCH", dt_utc, True, [f"P{l//24}DT{l%24}H" for l in ore])
        try:
            print(f"  ⬇️  Scarico dati CLCH per {len(ore)} ore...")
        ...
        for h in ore:
            ...
            ch.title(f"ICON-CH2 EPS - Cloud Cover High (%)\nRun: {dt_utc.strftime('%d/%m/%Y %H:%M UTC')} | Ore {(dt_loc + timedelta(hours=h)).strftime('%H:%M del %d/%m')}"); ch.legend(label="CLCH (%)")
            ch.save(f"h_{h}.png"); fp.append(f"h_{h}.png"); plt.close(ch.fig)
        invia_album(fp, f"ICON-CH2 EPS: Dettaglio Nubi Alte (CLCH)\n{bn}\nRun {n_run}")
