...
LATITUDE = 45.07
LONGITUDE = 7.54
FILE_LAST_HOUR = "ultima_ora_icon_ch2_lcl.txt"
RUN_DURATION = 120
START_DELAY = 1
...
def invia_album_telegram(file_paths: list, caption: str):
    ...
    thread_id = os.getenv("TELEGRAM_THREAD_ID_27")
...
def genera_album_orari(dt_run_utc: datetime, nome_run: str):
    ...
    # LCL scale 0 - 4000 (salti di 400)
    my_levels = [0, 400, 800, 1200, 1600, 2000, 2400, 2800, 3200, 3600, 4000]
    my_colors = ["#a50026", "#d73027", "#f46d43", "#fdae61", "#fee090", "#e0f3f8", "#abd9e9", "#74add1", "#4575b4", "#313695"]
    ...
    for block_name, ore_list in blocchi.items():
        ...
        try:
            print(f"  ⬇️  Scarico dati LCL_ML per {len(ore_list)} ore...")
        ...
        for h in ore_list:
            # Calcolo istantaneo, nessuna sottrazione necessaria
            ...
            title = f"ICON-CH2 EPS - LCL Mean (m)\nRun: {dt_run_utc.strftime('%d/%m/%Y %H:%M UTC')} | {str_valida}"
            chart.title(title)
            chart.legend(label="LCL_ML (m)")
        ...
        caption_album = f"ICON-CH2 EPS: Dettaglio LCL\n{block_name}\nRun {nome_run}"
...
def main():
    ...
    if is_new:
        print(f"🚀 Lancio generazione Album LCL ICON-CH2 per il RUN {nome_run} ({dt_run_utc})")
