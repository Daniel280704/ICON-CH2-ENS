...
LATITUDE = 45.07
LONGITUDE = 7.54
FILE_LAST_HOUR = "ultima_ora_icon_ch2_cin_mu.txt"
...
def estrai_limiti_run(hourly_data: dict, ref_param: str) -> tuple[bool, str, datetime]:
    ...
    if os.path.exists(FILE_LAST_HOUR):
        ...
        if ultima_ora_valida_str <= ultima_ora_salvata:
            print(f"✅ Run ICON-CH2 CIN_MU {nome_run} già elaborato.")
            return False, "", None
...
def invia_album_telegram(file_paths: list, caption: str):
    ...
    # Aggiornato per utilizzare il Thread ID 10
    thread_id = os.getenv("TELEGRAM_THREAD_ID_10")
...
def genera_album_cin_mu(dt_run_utc: datetime, nome_run: str):
    rome_tz = pytz.timezone("Europe/Rome")
    ...
    # Scala colori CIN_MU adattata (inibizione): 10-1000 J/kg (16 livelli, 15 colori)
    ...
    for block_name, ore_list in blocchi.items():
        print(f"\n📊 Generazione album CIN_MU: {block_name}")
        
        lead_times_str = [f"P{l // 24}DT{l % 24}H" for l in ore_list]

        # Richiesta alla API per CIN_MU
        req = ogd_api.Request(
        ...
        try:
            print(f"  ⬇️  Scarico dati CIN_MU per {len(ore_list)} ore...")
        ...
        for h in ore_list:
            cin_step = cin_mean.sel(lead_time=np.timedelta64(h, 'h'))
            cin_geo = regrid.iconremap(cin_step, destination)

            chart = earthkit.plots.Map(domain=domain)
            chart.grid_cells(cin_geo, x="lon", y="lat", style=Style(colors=my_colors, levels=my_levels))
            ...
            chart.title(f"ICON-CH2 EPS - CIN_MU (J/kg)\nRun: {dt_run_utc.strftime('%d/%m/%Y %H:%M UTC')} | {str_valida}")
            chart.legend(label="CIN_MU (J/kg)")
            
            f_name = f"cin_mu_{h}.png"
            chart.save(f_name)
        ...
        if percorsi_foto:
            caption_album = f"ICON-CH2 EPS: CIN_MU (Inibizione Connettiva)\n{block_name}\nRun {nome_run}"
...
def main():
    ...
    if is_new:
        print(f"🚀 Lancio generazione Album CIN_MU per il RUN {nome_run} ({dt_run_utc})")
        genera_album_cin_mu(dt_run_utc, nome_run)
