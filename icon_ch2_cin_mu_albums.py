...
def genera_album_cin_mu(dt_run_utc: datetime, nome_run: str):
    ...
    try:
        print(f"  ⬇️  Scarico dati CIN_MU per le 120 ore...")
        cin_data = scarica_variabile_con_retry(req)
    ...
    for day_str, intervals in intervals_by_day.items():
        ...
        for h_start, h_end in intervals:
            ...
            cin_interval = cin_data.sel(lead_time=hours_slice).max(dim="lead_time").mean(dim="eps")

            cin_geo = regrid.iconremap(cin_interval, destination)
            ...
            title = f"ICON-CH2 EPS - CIN_MU MAX (Media dei Massimi)\n{day_str} | Fascia {orario_str}\nRun: {dt_run_utc.strftime('%d/%m %H:%M UTC')}"
            
            chart.title(title)
            chart.legend(label="CIN_MU (J/kg)")

            filename = f"cin_mu_max_{h_start}_{h_end}.png"
            chart.save(filename)
            percorsi_foto.append(filename)
            
            plt.close(chart.fig)
        
        caption_album = f"ICON-CH2 EPS: CIN_MU (Inibizione Connettiva) MAX\nFasce triorarie del {day_str}\nRun {nome_run}"
