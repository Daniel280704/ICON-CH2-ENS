...
LATITUDE = 45.07
LONGITUDE = 7.54
FILE_LAST_HOUR = "ultima_ora_icon_ch2_grau_prob.txt"
RUN_DURATION = 120
START_DELAY = 1
...
def invia_album_telegram(file_paths: list, caption: str):
    ...
    thread_id = os.getenv("TELEGRAM_THREAD_ID_35")
...
def genera_album_orari(dt_run_utc: datetime, nome_run: str):
    ...
    # Livelli e colori percentuali uniformati
    ...
    for block_name, ore_list in blocchi.items():
        ...
        try:
            tot_grau = ogd_api.get_from_ogd(req)
        except Exception as e:
        ...
        for h in ore_list:
            if h == 1:
                grau_diff = tot_grau.sel(lead_time=np.timedelta64(h, 'h'))
            else:
                grau_diff = tot_grau.sel(lead_time=np.timedelta64(h, 'h')) - tot_grau.sel(lead_time=np.timedelta64(h-1, 'h'))

            prob_xr = (grau_diff >= 0.5).astype(float).mean(dim="eps") * 100
            
            grau_geo = regrid.iconremap(prob_xr, destination)
            grau_geo = grau_geo.where(grau_geo >= 10)
            ...
            title = f"ICON-CH2 EPS - Probabilità Graupel >= 0.5 mm/h (%)\nRun: {dt_run_utc.strftime('%d/%m/%Y %H:%M UTC')} | {str_valida}"
            chart.title(title)
        ...
        caption_album = f"ICON-CH2 EPS: Probabilità Graupel oraria >= 0.5 mm\n{block_name}\nRun {nome_run}"
...
def main():
    ...
    if is_new:
        print(f"🚀 Lancio generazione Probabilità Oraria Graupel per il RUN {nome_run} ({dt_run_utc})")
