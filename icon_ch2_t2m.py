...
LATITUDE, LONGITUDE, FILE_LAST_HOUR = 45.07, 7.54, "ultima_ora_icon_ch2_t2m.txt"
...
def invia_album(file_paths, caption):
    t, c, th = os.getenv("TELEGRAM_TOKEN"), os.getenv("TELEGRAM_CHAT_ID"), os.getenv("TELEGRAM_THREAD_ID_34")
...
def genera(dt_utc, n_run):
    ...
    for bn, ore in raggruppa(dt_loc).items():
        try: vm = ogd_api.get_from_ogd(ogd_api.Request("ogd-forecasting-icon-ch2", "T_2M", dt_utc, True, [f"P{l//24}DT{l%24}H" for l in ore])).mean(dim="eps")
        except: continue
        fp = []
        for h in ore:
            val_celsius = vm.sel(lead_time=np.timedelta64(h, 'h')) - 273.15
        ...
        invia_album(fp, f"ICON-CH2 EPS: Dettaglio Temperatura 2m\n{bn}\nRun {n_run}")
