...
LATITUDE = 45.07
LONGITUDE = 7.54
FILE_LAST_HOUR = "ultima_ora_icon_ch2_snow_depth_24h.txt"
RUN_DURATION = 120
START_DELAY = 1
...
def estrai_limiti_run(hourly_data: dict, ref_param: str, utc_offset_sec: int) -> tuple[bool, str, datetime]:
    ...
    if os.path.exists(FILE_LAST_HOUR):
        ...
        if ultima_ora_valida_str <= ultima_ora_salvata:
            print(f"✅ Run ICON-CH2 {nome_run} (Snow Depth) già elaborato (Ultimo blocco: {ultima_ora_valida_str}).")
            return False, "", None
...
def invia_album_telegram(file_paths: list, caption: str):
    ...
    thread_id = os.getenv("TELEGRAM_THREAD_ID_26")
...
def genera_album_24h(dt_run_utc: datetime, nome_run: str):
    ...
    for h_start, h_end in intervals:
        ...
        snow_diff = snow_end - snow_start

        snow_geo = regrid.iconremap(snow_diff, destination)

        chart = earthkit.plots.Map(domain=domain)
        chart.grid_cells(snow_geo, x="lon", y="lat", style=Style(colors=my_colors, levels=my_levels))
        ...
        title = f"ICON-CH2 EPS - Variazione Manto Nevoso (cm)\nRun: {dt_run_utc.strftime('%d/%m/%Y %H:%M UTC')} | {str_valida}"
        
        chart.title(title)
        chart.legend(label="Variazione Spessore Neve (cm)")

        filename = f"snow_depth_var_{h_start}_{h_end}.png"
        chart.save(filename)
        percorsi_foto.append(filename)
        
        plt.close(chart.fig)
    
    caption_album = f"ICON-CH2 EPS: Variazione Manto Nevoso al suolo\nRun {nome_run}"
    invia_album_telegram(percorsi_foto, caption_album)
    
    for f in percorsi_foto:
        if os.path.exists(f): os.remove(f)
    del tot_snow, snow_mean

def main():
    print("Cerco l'ultimo run completo ICON-CH2 via Open-Meteo per Snow Depth...")
    ...
    if is_new:
        print(f"🚀 Lancio generazione Album giornalieri Variazione Neve per il RUN {nome_run} ({dt_run_utc})")
